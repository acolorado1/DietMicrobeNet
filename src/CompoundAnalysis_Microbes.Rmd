...
taxonomy_meta <- meta %>% 
  ...
  distinct(compound, organisms, .keep_all = T) %>% #there could be repeats due to food items
...
for (i in seq_along(genera)) {
  for (j in seq_len(i)) {  # compute only for j ≤ i
    genus1 <- genera[i]
    genus2 <- genera[j]
