...
packages <- c("dplyr", "ggplot2", "readr", "cowplot", "thematic", "tidyr",
              "tibble", "plotly", "rmarkdown", "ggpubr", "stringr")
...
meta_clean$assoc_food <- stringr::str_trim(meta_clean$assoc_food)
```
...
plot_ly(data = plot_df, 
        ...
        type = "pie", 
        sort = F) %>% 
  layout(title = "Number of Compounds Per Food Item")
```
</center>
...
ggplot(unique_compounds, aes(x = reorder(groups, unique_count), y = unique_count)) + 
  ...
  annotate("text", 
           x = length(unique_compounds$groups)/4, 
           y = max(unique_compounds$unique_count)/2 , 
